...
process.env.GOOGLE_APPLICATION_CREDENTIALS = "./config/dotted-tide-453000-f3-2c7e9aae9a03.json";
...
app.post('/process-image', async (req, res) => {
    try {
        ...
        const keywords = text.toLowerCase().replace(/[^a-z0-9\s]/g, '').split(/\s+/).filter(word => word.length > 3);
        ...
        matchingWines = matchingWines.map(wine => {
            ...
            wineData.forEach(({ value, weight }) => {
                if (value) {
                    const matchCount = keywords.filter(k => value.toLowerCase().includes(k)).length;
        ...
        matchingWines.sort((a, b) => b.score - a.score || b.avgRating - a.avgRating);
